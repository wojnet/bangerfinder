...
namespace FinalAppTemplate.Controllers
{
    ...
    public class UserCreationController : ControllerBase
    {
        ...
        // Add to UserCreationController.cs
        [HttpGet("lastfm-connect-url")]
        public IActionResult GetLastFmUrl()
        {
            var apiKey = Configuration["LastFm:ApiKey"];
            // Point this to your actual LastFmAuth callback endpoint!
            var callback = "http://localhost:3000/lastfm-callback";

            var url = $"http://www.last.fm/api/auth/?api_key={apiKey}&cb={Uri.EscapeDataString(callback)}";
            return Ok(new { url });
        }
